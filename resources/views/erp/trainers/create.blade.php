...
@section('page_title', 'Add trainer')

@section('content')
    <div class="panel-card">
        <div class="panel-heading">Trainer</div>
        <div class="panel-body p-4">
            <form method="post" action="{{ route('erp.trainers.store') }}" class="mx-auto" style="max-width:560px">
                @csrf
                <div class="mb-3">
                    <label class="form-label">Name <span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-control rounded-3" required value="{{ old('name') }}">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="role_title" class="form-control rounded-3" value="{{ old('role_title') }}">
                </div>
                <div class="mb-3">
                    <label class="form-label">Salary mode <span class="text-danger">*</span></label>
                    <select name="salary_mode" class="form-select rounded-3">
                        <option value="fixed" @selected(old('salary_mode') === 'fixed')>Fixed monthly</option>
                        <option value="attendance" @selected(old('salary_mode') === 'attendance')>Per weekday in month
                            × rate</option>
                    </select>
                </div>
                <div class="mb-3">
                ...
                <a href="{{ route('erp.trainers.index') }}" class="btn btn-link">Cancel</a>
            </form>
        </div>
    </div>
@endsection
